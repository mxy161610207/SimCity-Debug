...
public class AppMonitor implements EventListener{
	public void eventTriggered(Event event) {
		...
		case DELIVERY_RELEASED:
			PkgHandler.send(new AppPkg().setDelivery(event.dtask.id, null, event.dtask.src.name, event.dtask.dst.name, event.dtask.phase));
			break;
		case DELIVERY_COMPLETED:
			PkgHandler.send(new AppPkg().setDelivery(event.dtask.id, event.dtask.car.name, event.dtask.src.name, event.dtask.dst.name, event.dtask.phase));
			break;
